...
import {
    ...
    Platform,
    Alert,
    ScrollView, // Added ScrollView for better content management
} from 'react-native';
import { useDispatch } from 'react-redux';
import { addNoteToEvent } from "../redux/slices/eventsSlice";
import { StackScreenProps } from '@react-navigation/stack'; // For better type safety with navigation

// Define the types for your navigation stack parameters
// Assuming your root stack has a type defined like this
type RootStackParamList = {
    // ... other screen params
    AddNoteScreen: { eventId: string; selectedDate: string };
    // ... potentially other screens
};

type AddNoteScreenProps = StackScreenProps<RootStackParamList, 'AddNoteScreen'>;

function AddNoteScreen({ navigation, route }: AddNoteScreenProps) {
    const dispatch = useDispatch();
    const { eventId, selectedDate } = route.params;

    const [noteText, setNoteText] = useState<string>(''); // Explicitly type useState as string
    ...
    return (
        <SafeAreaView style={styles.safeArea}>
            ...
            >
                ...
                <View style={styles.headerBar}>
                    <TouchableOpacity onPress={() => navigation.goBack()} style={styles.backButton}>
                        {/* Ensure your g69.png path is correct */}
                        <Image
                            source={require('../assets/img/g69.png')}
                            style={styles.backIcon}
                        />
                ...
                <ScrollView contentContainerStyle={styles.scrollViewContent}>
                    ...
                    <View style={styles.inputContainer}>
                        <TextInput
                            ...
                            multiline
                            placeholder="Start typing your thoughts here..."
                            placeholderTextColor="#C5A0D6" // Lighter violet for placeholder
                            value={noteText}
                            onChangeText={setNoteText}
                            autoCorrect={true}
                            spellCheck={true}
...
const styles = StyleSheet.create({
    ...
    promptText: {
        fontSize: 24,
        fontWeight: 'bold',
        color: '#8A2BE2', // BlueViolet for prompt text
        textAlign: 'center',
        marginBottom: 10,
        lineHeight: 32,
        fontFamily: 'Fredoka',
    ...
    saveNoteButton: {
        backgroundColor: '#9370DB', // MediumPurple for the save button
        borderRadius: 30,
        paddingVertical: 18,
        alignItems: 'center',
        justifyContent: 'center',
